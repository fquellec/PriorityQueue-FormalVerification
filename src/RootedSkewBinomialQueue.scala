...
object PriorityQueue{
  ...
  **/
  case class RootedPriorityQueue[T](elems: Heap[T], comparator: Ordering[T]) extends PriorityQueue[T]{   
    ...
    def insert(x: T): RootedPriorityQueue[T] = this.elems match {
      ...
      case Nodes(r, ns) => {
        if(comparator.compare(x,r.value) <= 0){
           RootedPriorityQueue(Nodes(Node(x, 0, Empty), SkewBinomialQueue(ns, this.comparator).insert(r.value).elems), this.comparator)
        }else{
    ...
    def deleteMin: PriorityQueue[T] = {
      require(!this.isEmpty)
      this.elems match { // there is at least root
        case Nodes(r,ns) => ns match {
          case Empty => RootedPriorityQueue(Empty,this.comparator)
          case ns => {
            val pq = SkewBinomialQueue(ns,this.comparator)
            val min = pq.findMin
            RootedPriorityQueue(Nodes(Node(min,0,Empty),pq.deleteMin.asInstanceOf[SkewBinomialQueue[T]].elems),this.comparator)
            }
        }
      }      
    }
  ...
  def meld(that: PriorityQueue[T]): PriorityQueue[T] = this.elems match{
        ...
        case RootedPriorityQueue(elems_,_) => elems_ match {
          ...
          case Nodes(x2,q2) => {
            ...
            if(comparator.compare(x1.value,x2.value) <= 0){
              ...
              RootedPriorityQueue(Nodes(x1, pq1.meld(pq2).insert(x2.value).asInstanceOf[SkewBinomialQueue[T]].elems),this.comparator)
            }else{
              RootedPriorityQueue(Nodes(x2, pq1.meld(pq2).insert(x1.value).asInstanceOf[SkewBinomialQueue[T]].elems),this.comparator)
            }
          } 
        }
    }
    ...
    def toList = this.elems match{
      ...
      case Nodes(r,ns) => {
      val pq = SkewBinomialQueue(ns,this.comparator)
      sort(r.value :: pq.toList)(this.comparator)
      }
    }
  } 
}
