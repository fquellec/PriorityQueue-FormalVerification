...
object PriorityQueue{
  ...
  */
  sealed abstract class PriorityQueue[T]{
    ...
    def insert(x: T) = (??? : PriorityQueue[T])
      .ensuring(res => res.toList.zip(SInsert(x, this.toList)(this.comp)).forall{x => this.comp.compare(x._1, x._2) == 0})
    
    def meld(that: PriorityQueue[T]) = (??? : PriorityQueue[T])
      .ensuring(res => res.toList.zip(this.toList ++ that.toList).forall{x => this.comp.compare(x._1, x._2) == 0})
  ...
  **/
  case class BinomialQueue[T](elems: Heap[T], comparator: Ordering[T]) extends PriorityQueue[T]{
    //require(SortedList.isSorted(elems) && SoundNode.soundNode(elems)(comparator))
    ...
    def insert_(y: Node[T], nodes: Heap[T]): Heap[T] = {
      ...
      nodes match {
        case Empty => 
          //assert(heapToList(Nodes(y, Empty)).size == 1)
          //assert(heapToList(Nodes(y, Empty)) == nodeToLis)
          //assert(heapToList(Nodes(y, Empty)).zip(sort(nodeToList(y) ++ heapToList(nodes))(this.comp)).forall{x => this.comp.compare(x._1, x._2) == 0})
          Nodes(y, Empty)
        case Nodes(n, ns) => {  
          if (y.rank < n.rank) 
            Nodes(y, Nodes(n, ns)) 
          else {
            insert_(link_(n, y), ns)
          }
        }
      } 
    }.ensuring(res => heapToList(res).zip(sort(nodeToList(y) ++ heapToList(nodes))(this.comp)).forall{x => this.comp.compare(x._1, x._2) == 0})
    ...
    def toList = {sort(heapToList(this.elems))(this.comp)}.ensuring(isSorted(_)(this.comp))
  }
}